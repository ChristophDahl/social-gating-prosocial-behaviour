...
%% Figure 1: realised helping by task and parameter regime
...
if showTitles
    title('Realised helping by task regime');
end
...
%% Figure 2: failure decomposition by task, averaged across parameter regimes
...
if showTitles
    title('Failure decomposition by task regime');
end
xticks(1:numel(taskKeep));
xticklabels(taskLabels);
xtickangle(32);
...
%% Figure 3: failure decomposition separately for each parameter regime
for j = 1:numel(systemKeep)
...
%PRETTYSYSTEMLABELS Manuscript-friendly parameter-regime labels.
